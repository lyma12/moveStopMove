...
public static class SimplePool
{
    ...
    static public T Spawn<T>(T obj, Vector3 pos, Quaternion rot, Transform parent) where T : GameUnit{
        if (!poolInstanceID.ContainsKey(obj.GetInstanceID()))
        {
            Preload(obj, 1, parent, true, false);
        }
        T gameUnit = poolInstanceID[obj.GetInstanceID()].Spawn(pos, rot) as T;
